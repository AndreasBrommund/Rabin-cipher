...
class BlockCipher:
    ...
    def encrypt(self, message: str, iv: int = 0) -> [int]:
        ...
        if self.mode == CipheringMode.ECB:
            return CipheringMode.encrypt_ecb(int_blocks, self)
        elif self.mode == CipheringMode.CBC:
            return CipheringMode.encrypt_cbc(int_blocks, iv, self)
...
class CipheringMode(Enum):
    ...
    @staticmethod
    def encrypt_ecb(message_blocks: [int], block_cipher: BlockCipher) -> [int]:

        ans = [0] * len(message_blocks)

        for i, b in enumerate(message_blocks):
            ans[i] = block_cipher.encryption_function(b)
        return ans
    ...
    @staticmethod
    def encrypt_cbc(int_blocks: [int], iv: int, block_cipher: BlockCipher) -> [int]:
